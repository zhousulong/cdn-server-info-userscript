...
// @version      7.56.27
// @description  [v7.56.27] 修复反复点击面板导致位移的 bug（增加5px拖拽阈值）；优化折叠/展开动画（spring曲线）；修复折叠时悬停显示按钮的问题；修复展开状态顶部留白问题。
// @description:en [v7.56.27] Fix panel drift on repeated clicks (5px drag threshold); improve collapse/expand animation (spring curve); fix buttons appearing on hover in collapsed state; fix extra whitespace in expanded state.
// @author       Zhou Sulong
// @license      MIT
// @match        *://*/*
...
// @resource     cdn_rules https://raw.githubusercontent.com/zhousulong/cdn-server-info-userscript/main/cdn_rules.json?v=7.56.27
...
(function () {
    ...
    const customHandlers = {
        Akamai: {
            getInfo: (h, rule) => {
                ...
                const extra = requestId
                    ? `Req-ID: ${requestId}`
                    : 'Detected via Akamai header/cookie';
            ...
            getInfo: (h, rule) => {
                ...
                if (ctlStatus) {
                    ...
                    if (match) {
                        ...
                        pop = (
                            parts[0] + (parts[1] ? '-' + parts[1].replace(/\d+$/, '') : '')
                        ).toUpperCase();
                    }
                }
        ...
        Zenlayer: {
            getInfo: (h) => {
                let cache = h.get('hittype') || 'N/A';
                if (cache !== 'N/A') {
                    cache = cache.replace('TCP_', '').toUpperCase();
                } else {
                    cache = getCacheStatus(h);
                }
                ...
                if (via) {
                    ...
                    for (let i = parts.length - 1; i >= 0; i--) {
                        const match = parts[i].match(/[A-Z]{2}\.([A-Z]{3})\d+/i);
                        if (match && match[1]) {
                            pop = match[1].toUpperCase();
                            break;
                        }
                ...
                const requestId = h.get('x-tiggee') || 'N/A';
            ...
            if (val) {
                if (rule.pop_regex) {
                    ...
                    if (letterMatch && letterMatch[1].length >= 3) {
                        ...
                        pop = letterMatch[1]
                            .substring(0, Math.min(4, letterMatch[1].length))
                            .toUpperCase();
                    } else {
    ...
    function parseInfo(response) {
        ...
        for (const [name, rule] of Object.entries(cdnRules)) {
            ...
            if (rule.server) {
                ...
                if (server && new RegExp(rule.server, 'i').test(server)) {
                    ...
                    const isUnique = uniqueServers.some((sig) =>
                        new RegExp(sig, 'i').test(rule.server)
                    );
            ...
            if (score > 0) {
                ...
                const handler = customHandlers[name]
                    ? customHandlers[name].getInfo
                    : genericGetInfo;
                candidates.push({
        ...
        if (candidates.length > 0) {
            ...
            if (candidates.length > 1) {
                console.log(
                    `[CDN Scoring] Winner: ${winner.provider} (${winner.score}) vs Runner-up: ${candidates[1].provider} (${candidates[1].score})`
                );
            }
    ...
    const cdnIcons = {
        ...
        Netlify:
            '<svg viewBox="0 0 256 226" preserveAspectRatio="xMidYMid"><path fill="currentColor" d="M69.181 188.087h-2.417l-12.065-12.065v-2.417l18.444-18.444h12.778l1.704 1.704v12.778zM54.699 51.628v-2.417l12.065-12.065h2.417L87.625 55.59v12.778l-1.704 1.704H73.143z"/><path fill="currentColor" d="M160.906 149.198h-17.552l-1.466-1.466v-41.089c0-7.31-2.873-12.976-11.689-13.174-4.537-.119-9.727 0-15.274.218l-.833.852v53.173l-1.466 1.466H95.074l-1.466-1.466v-70.19l1.466-1.467h39.503c15.354 0 27.795 12.441 27.795 27.795v43.882l-1.466 1.466Z"/><path fill="currentColor" d="M71.677 122.889H1.466L0 121.423V103.83l1.466-1.466h70.211l1.466 1.466v17.593zM254.534 122.889h-70.211l-1.466-1.466V103.83l1.466-1.466h70.211L256 103.83v17.593zM117.876 54.124V1.466L119.342 0h17.593l1.466 1.466v52.658l-1.466 1.466h-17.593zM117.876 223.787v-52.658l1.466-1.466h17.593l1.466 1.466v52.658l-1.466 1.465h-17.593z"/></svg>',
        ...
        VergeCloud:
            '<svg viewBox="0 0 44 44"><path fill="currentColor" fill-opacity="0.95" d="M21.81,43.99c-.89,0-1.79-.05-2.68-.16-5.81-.7-10.98-3.62-14.56-8.22C-2.83,26.11-1.06,12.27,8.52,4.75,13.2,1.09,19-.55,24.87.16c5.81.7,10.98,3.62,14.56,8.22,7.4,9.51,5.63,23.35-3.95,30.86-3.96,3.11-8.74,4.76-13.67,4.76h0ZM22.2,4.43c-3.92,0-7.71,1.31-10.87,3.79-7.67,6.02-9.1,17.11-3.18,24.72,2.84,3.65,6.94,5.96,11.54,6.52,4.65.56,9.27-.74,12.99-3.66,7.67-6.02,9.1-17.11,3.18-24.72-2.84-3.65-6.94-5.96-11.54-6.52-.71-.09-1.41-.13-2.12-.13Z"/><path fill="currentColor" fill-opacity="0.75" d="M7.33,34.27c-1.17-2.4-1.76-5.06-1.7-7.82.1-4.66,2.05-9.07,5.46-12.39,7.08-6.89,18.39-7,25.2-.24,6.81,6.76,6.59,17.87-.5,24.76-3.42,3.33-7.92,5.19-12.66,5.25-4.78.06-9.22-1.72-12.53-5.01-1.36-1.35-2.45-2.88-3.27-4.55h0ZM35.49,20.26c-.59-1.21-1.39-2.33-2.4-3.33-5.06-5.03-13.51-4.9-18.83.27-2.59,2.52-4.06,5.84-4.13,9.35-.08,3.48,1.22,6.73,3.67,9.16,2.44,2.43,5.74,3.74,9.28,3.7,3.57-.04,6.96-1.45,9.55-3.97,4.26-4.15,5.25-10.3,2.86-15.17h0Z"/><path fill="currentColor" fill-opacity="0.55" d="M29.5,33.52c-5.78,4.63-14.32,3.78-19.03-1.9-4.71-5.68-3.84-14.07,1.94-18.7,5.78-4.63,14.32-3.78,19.03,1.9,4.71,5.68,3.84,14.07-1.94,18.7ZM15.25,16.34c-3.86,3.09-4.44,8.69-1.29,12.48,3.14,3.79,8.84,4.36,12.7,1.27,3.86-3.09,4.44-8.69,1.29-12.48-3.14-3.79-8.84-4.36-12.7-1.27Z"/></svg>',
        Zenlayer:
            '<svg viewBox="0 0 31.8 31.9"><path fill="currentColor" fill-opacity="0.75" d="M19.3 3.9c-1-.4-2.1-.7-3.3-.7H15.4C9.8 3.2 5.2 7.8 5.2 13.4c0 1.2.2 2.5.7 3.6v-.5c0-5.2 4.2-9.4 9.4-9.4s9.4 4.2 9.4 9.4c0 .5 0 1.1-.1 1.6 0 .3-.1.6-.2.9-.3 1-.7 2-1.3 2.9-3.6 5.8-11.3 7.6-17.1 4-3.7-2.2-5.9-6.1-6-10.4v.4c0 8.8 7.2 16 16 16 2 0 3.9-.4 5.8-1.1 7.1-4 9.6-13 5.6-20.1-1.8-3.2-4.7-5.6-8.1-6.8z"/><path fill="currentColor" fill-opacity="0.55" d="M15.3 25.9c-5 0-9.1-3.9-9.4-8.9-.4-1.1-.7-2.4-.7-3.6 0-5.6 4.6-10.2 10.2-10.2H16h-1.7C7.9 3.2 2.7 6.8.4 12.4c0 0-.1.2-.1.4-.2.8-.3 1.6-.3 2.4v.3c.1 6.9 5.8 12.4 12.7 12.3 4.3-.1 8.2-2.3 10.4-5.9-1.7 2.5-4.8 4-7.8 4z"/><path fill="currentColor" fill-opacity="0.95" d="M16 0C9.3 0 3.3 4.1 1 10.4c-.3.8-.6 1.6-.7 2.4 0-.1.1-.4.1-.4 2.3-5.6 7.8-9.3 13.9-9.2H16c1.1.1 2.2.3 3.2.7 7.7 2.6 11.9 11 9.3 18.7-1.2 3.4-3.6 6.3-6.7 8.1C30 27.5 33.9 18.2 30.6 10 28.2 4 22.4.1 16 0z"/></svg>',
        Quantil:
            ...
            try {
                ...
                if (result && result.Answer) {
                        ...
                        for (const [providerName, rule] of Object.entries(cdnRules)) {
                            if (rule.cnames && Array.isArray(rule.cnames)) {
                                for (const pattern of rule.cnames) {
                                    ...
                                    const isSuffix =
                                        cleanCname.toLowerCase().endsWith('.' + patternLower) ||
                    ...
                    if (candidates.length > 0) {
                        ...
                        const winner = candidates[0];
                        console.log(
                            `[CDN DNS] Confirmed ${winner.provider} via CNAME: ${winner.cname} (Priority: ${winner.priority})`
                        );
                        if (candidates.length > 1) {
                            console.log(
                                `[CDN DNS] Runner-up: ${candidates[1].provider} (Priority: ${candidates[1].priority})`
                            );
                        }
                        ...
                        console.log(
                            `[CDN DNS] Found CNAME(s) but no matching CDN: ${foundCnames.join(', ')}`
                        );
    ...
    function updatePanelWithDNS(dnsResult, currentInfo) {
        ...
        if (dnsResult.provider && dnsResult.provider !== currentInfo.provider) {
            ...
            console.log(
                `[CDN DNS] ⚠️ Correcting provider from ${currentInfo.provider} to ${dnsResult.provider}`
            );
            ...
            if (dnsResult.provider) {
                console.log(
                    `[CDN DNS] ✓ Confirmed ${dnsResult.provider} via CNAME: ${dnsResult.cname}`
                );
            } else {
    ...
    function detectPageTheme() {
        try {
            ...
            if (
                combinedClasses.includes('dark-mode') ||
                combinedClasses.includes('dark-theme') ||
                combinedClasses.includes(' dark ') ||
                combinedClasses.startsWith('dark ') ||
                combinedClasses.endsWith(' dark')
            ) {
                return 'dark';
            }
            if (
                combinedClasses.includes('light-mode') ||
                combinedClasses.includes('light-theme') ||
                combinedClasses.includes(' light ') ||
                combinedClasses.startsWith('light ') ||
    ...
    function getPanelCSS() {
        ...
        #cdn-info-panel-enhanced.collapsed {
            ...
            /* transform-origin will be set dynamically via JavaScript */
        }
    ...
    function createDisplayPanel(info) {
        ...
        const watermarkHtml = watermarkSvg
            ? `<div class="cdn-watermark">${watermarkSvg}</div>`
            : '';
        ...
        makeDraggable(host, panel); // Enabled
    }
    ...
    function setupScrollCollapse(host, panelElement) {
        ...
        // Set transform origin based on panel position in viewport
        function setTransformOrigin() {
            const rect = host.getBoundingClientRect();
            const viewportWidth = window.innerWidth;
            const viewportHeight = window.innerHeight;

            // Use panel CENTER to determine position (not top-left corner)
            // This ensures position detection is consistent when panel size changes
            ...
            // User requested to ALWAYS expand towards top-left
            // This means we must always anchor the origin to the bottom-right corner
            let originX = 'right';
            let originY = 'bottom';
            ...
            console.log(
                `[CDN Panel] Center: (${Math.round(centerX)}, ${Math.round(centerY)}), Position: ${isRight ? 'Right' : 'Left'}-${isBottom ? 'Bottom' : 'Top'}, Origin: ${origin}`
            );
        }
        ...
        panelElement.addEventListener('click', (e) => {
            ...
            if (
                ...
                e.target.classList.contains('theme-btn')
            ) {
                return;
            }
    ...
    async function runExecution(retriesLeft) {
        ...
        try {
            ...
            if (response.status === 403 || response.status === 405) {
                console.log(
                    `[CDN Detector] HEAD returned ${response.status}, retrying with GET...`
                );
        ...
        if (typeof GM_getValue !== 'undefined') {
            try {
                ...
                if (savedSettings) {
                    ...
                    console.log(
                        '[CDN Detector] No saved settings, using defaults:',
                        config.settings
                    );
            ...
            themeCheckTimeout = setTimeout(() => {
                if (config.settings.theme === 'auto' || !config.settings.theme) {
                    ...
                    if (currentPageTheme && currentPageTheme !== lastPageTheme) {
                        console.log(
                            `[CDN Detector] Page theme changed: ${lastPageTheme} -> ${currentPageTheme}`
                        );
