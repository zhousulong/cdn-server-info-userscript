...
// @version      7.11.1
// @description  [v7.11.1] Renamed Tianyi Cloud to State Cloud and updated logo. Improved CDNetworks identification.
// @description:en [v7.11.1] Renamed Tianyi Cloud to State Cloud and updated logo. Improved CDNetworks identification.
// @author       Zhou Sulong
// @license      MIT
// @match        *://*/*
...
// @resource     cdn_rules https://raw.githubusercontent.com/zhousulong/cdn-server-info-userscript/main/cdn_rules.json?v=7.11.1
        ...
        'CDNetworks': {
            getInfo: (h, rule) => {
                ...
                if (via) {
                    ...
                    const match = via.match(/PS-([A-Z]{3})-/);
                    if (match) {
                        pop = match[1].toUpperCase();
                    }
                }

                // If not found in via, try x-px header
                if (pop === 'N/A') {
                    const xpx = h.get('x-px');
                    if (xpx) {
                        // Extract from "ht PS-000-01fWO165000" -> first PS node
                        const match = xpx.match(/PS-([A-Z0-9]{3})-/);
                        if (match) pop = match[1].toUpperCase();
                    }
                }
